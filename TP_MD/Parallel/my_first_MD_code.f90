...
call MPI_Comm_size(MPI_COMM_WORLD,nprocs,ierr) ! Number of processors called fro MPIRUN
allocate(T_scanned(nprocs))


call MPI_Comm_rank(MPI_COMM_WORLD,ME,ierr)     ! Fills the ME variable with the processor's number


if (ME==0) then   ! Così non lo spamma nproc volte
do i = 1, COMMAND_ARGUMENT_COUNT()
    ...
    if (index(string_lec, '-TSCAN').ne.0)then
       write(*,*) 'T initial:'
       read(*,*) T_i 
       write(*,*) 'T final:'
       read(*,*) T_f
...
do i = 1,nprocs
    T_scanned = T_i + (i-1) * (T_f-T_i)/(nprocs-1)
enddo
...
!!! BROADCAST TUTTE LE VARIABILI

CALL MPI_BCAST(T_scanned, nprocs, MPI_REAL8, 0, MPI_COMM_WORLD, ierr)

CALL MPI_BCAST(filename, 100, MPI_CHARACTER, 0, MPI_COMM_WORLD, ierr)
CALL MPI_BCAST(timestep, 1, MPI_REAL8, 0, MPI_COMM_WORLD, ierr)
CALL MPI_BCAST(max_md_Step, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
...
CALL MPI_BCAST(thermo, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
...
temp_0 = T_scanned(ME+1)


write(real_str, '(F10.4)') temp_0

real_str = trim(real_str)



output_file = 'md_' // real_str // '.xyz'
energy_file = 'ENERGIES_' // real_str
